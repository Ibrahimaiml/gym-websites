...
import { OrbitControls, Float, MeshReflectorMaterial, PerspectiveCamera } from '@react-three/drei';
...
export default function Hero3DCanvas() {
  ...
  if (!mounted) {
    return (
      <div className="w-full h-[400px] md:h-[520px] flex flex-col items-center justify-center bg-dark-900/60 rounded-3xl border border-beast-500/20 backdrop-blur-md">
  ...
  return (
    <div 
      className="relative w-full h-[400px] md:h-[540px] rounded-3xl overflow-hidden glass-panel border border-beast-500/30 shadow-glow-gold group cursor-grab active:cursor-grabbing"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      {/* Dynamic Floating Badge */}
      <div className="absolute top-4 left-4 z-10 bg-dark-950/80 backdrop-blur-md px-3.5 py-1.5 rounded-full border border-beast-500/40 flex items-center space-x-2 shadow-lg">
      ...
      <Canvas gl={{ antialias: true }} dpr={[1, 2]}>
        ...
        <pointLight position={[-8, -5, -5]} intensity={2.5} color="#f59e0b" />
        <pointLight position={[5, -5, 5]} intensity={1.5} color="#10b981" />
        <spotLight position={[0, 10, 0]} intensity={2} angle={0.6} penumbra={1} color="#FFD700" />
      ...
      <div className="absolute bottom-4 right-4 z-10 bg-dark-950/80 backdrop-blur-md px-3 py-1.5 rounded-xl border border-gray-800 text-[11px] text-gray-300 flex items-center gap-1.5">
