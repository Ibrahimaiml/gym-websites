...
import { Dumbbell, ShieldAlert, Sparkles, RefreshCw, ZoomIn } from 'lucide-react';
    ...
    <section id="equipment" className="py-20 bg-dark-900/80 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-beast-500/10 border border-beast-500/30 text-beast-400 text-xs font-semibold uppercase tracking-wider mb-4">
            <Sparkles className="w-3.5 h-3.5" />
            <span>Interactive 3D Gear Visualizer</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tight">
            Inspect Our <span className="gold-gradient-text">World-Class Equipment</span>
          </h2>
          <p className="mt-4 text-gray-400 text-base md:text-lg">
            Experience our heavy-duty commercial workout gear in interactive WebGL 3D before your first workout at Keelaveethi, Madurai.
          </p>
        </div>
          ...
          {EQUIPMENT_DATA.map((item, idx) => (
            <button
              ...
              className={`px-5 py-3 rounded-2xl font-bold text-sm transition-all duration-300 flex items-center space-x-2 ${
                activeTab === idx
                  ? 'bg-gradient-to-r from-beast-500 to-beast-600 text-black shadow-glow-gold scale-105'
                  : 'bg-dark-800/80 text-gray-300 hover:text-white hover:bg-dark-700 border border-gray-800'
              }`}
            >
              <Dumbbell className={`w-4 h-4 ${activeTab === idx ? 'text-black' : 'text-beast-400'}`} />
              <span>{item.name}</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center glass-panel rounded-3xl p-6 md:p-8 border border-beast-500/20">
          {/* 3D Canvas Box (7 cols) */}
          <div className="lg:col-span-7 relative h-[380px] md:h-[480px] w-full rounded-2xl bg-dark-950/90 overflow-hidden border border-gray-800 shadow-2xl">
            {/* Control Toggles */}
              ...
              <button
                ...
                className={`p-2 rounded-xl border backdrop-blur-md text-xs font-semibold transition-all flex items-center space-x-1.5 ${
                  autoRotate
                    ...
                    : 'bg-dark-800/80 text-gray-400 border-gray-700'
                }`}
            ...
            {mounted ? (
              <Canvas gl={{ antialias: true }} dpr={[1, 2]}>
                ...
                <pointLight position={[-6, -4, -4]} intensity={2} color="#f59e0b" />
                <pointLight position={[6, -4, 4]} intensity={1.5} color="#ef4444" />
          ...
          {/* Equipment Technical Details (5 cols) */}
          <div className="lg:col-span-5 space-y-6">
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-beast-400">Gym Equipment Specification</span>
              <h3 className="text-2xl md:text-3xl font-extrabold text-white mt-1">{currentEquipment.name}</h3>
            ...
            <div className="pt-4">
              <a
                ...
                className="w-full inline-flex items-center justify-center space-x-3 px-6 py-4 rounded-2xl bg-gradient-to-r from-beast-500 via-beast-600 to-beast-amber text-black font-extrabold text-base shadow-glow-gold hover:opacity-95 transition-all transform hover:-translate-y-0.5"
              >
